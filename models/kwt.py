...
from utils.spec import mfcc_torch, adaptive_mel_filter, adaptive_mel_filter2
...
class FilterAttention(nn.Module):
    def __init__(self, output_size=40, d_model=116, num_heads=3, num_layers=1, d_ff=128, max_seq_length=98, dropout=0.1):
...
class KWT(nn.Module):
    def __init__(self, audio_settings, input_res, patch_res, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 1, dim_head = 64, dropout = 0., emb_dropout = 0., pre_norm = True, model_mode: int = 0, **kwargs):
        super().__init__()
    ...
    def create_filters(self, bs, fm, bw, do_norm=False):
        n_fft = self.audio_settings["n_fft"]
        x = torch.zeros((bs, n_fft // 2 + 1), device=self.device) # device=device
        x[:, :] = torch.arange(0, n_fft // 2 + 1).to(self.device)
        fm = (n_fft / 2 + 1) * fm
        bw = 4 * bw + 0.2
...
def kwt_from_name(model_name: str, audio_settings: dict):

...
def import_model(config, parallel=True):
    # model
    model = get_model(config["hparams"]["model"], audio_settings=config["hparams"]["audio"], model_mode=config["exp"]["model_mode"])
...
def import_optimization_methods(config, model, trainloader):
    # loss
    ...
    return optimizer, criterion, schedulers


def training_pipeline(config, model, optimizer, criterion, trainloader, valloader, schedulers):

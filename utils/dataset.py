...
class GoogleSpeechDataset(Dataset):
    ...
    def __init__(self, data_list: list, audio_settings: dict, label_map: dict = None, aug_settings: dict = None, cache: int = 0, model_mode: int = 0):
        super().__init__()
        ...
        if cache:
            ...
            self.data_list = init_cache(data_list, audio_settings["sr"], cache, audio_settings, model_mode=model_mode)
        else:
...
def cache_item_loader(path: str, sr: int, cache_level: int, audio_settings: dict, model_mode: int = 0) -> np.ndarray:
    ...
    if cache_level == 2:
        ...
        # x = melspectrogram(y=x, **audio_settings)
        # x = mfcc(S=x, n_mfcc=audio_settings["n_mels"], n_fft=audio_settings["n_fft"])
...
def init_cache(data_list: list, sr: int, cache_level: int, audio_settings: dict, n_cache_workers: int = 4, model_mode: int = 0) -> list:
    ...
    loader_fn = functools.partial(cache_item_loader, sr=sr, cache_level=cache_level, audio_settings=audio_settings, model_mode=model_mode)
...
def get_loader(data_list, config, train=True):
    ...
    dataset = GoogleSpeechDataset(
        ...
        model_mode=config["exp"]["model_mode"],
    )

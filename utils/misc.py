...
def get_model(model_config: dict, audio_settings: dict, model_mode: int = 0) -> nn.Module:
    ...
    if model_config["name"] is not None:
        return kwt_from_name(model_config["name"], audio_settings=audio_settings)
    else:
        model_config["model_mode"] = model_mode
        return KWT(**model_config, audio_settings=audio_settings)
